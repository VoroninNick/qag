    ...
    records_with_same_slug = another_records.select {|r|
      I18n.available_locales.each {|locale| I18n.with_locale(locale) {
        if !locale_errors.keys.include?(locale.to_sym) then
          locale_errors[locale.to_sym] = "#{locale} шлях повинен бути унікальним(id: #{r.id})"  if r.translations_by_locale.keys.include?(locale.to_sym) && r.translations_by_locale[locale.to_sym].url_fragment == record.translations_by_locale[locale.to_sym].url_fragment
        end
      }
      }
    }
